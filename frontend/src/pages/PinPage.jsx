            ...
            <div className="bg-white rounded-lg shadow-lg flex flex-wrap w-full max-w-4xl">
              <div className="w-full md:w-1/2 bg-gray-200 rounded-t-lg md:rounded-l-lg md:rounded-t-none  flex items-center justify-center">
                {pin.image && (
                  <img
                    src={pin.image.url}
                    alt=""
                    className="object-cover w-full rounded-t-lg md:rounded-l-lg lg:rounded-t-none lg:rounded-l-none"
                  ></img>
                )}
              </div>
              <div className="w-full md:w-1/2 p-6 flex flex-col  ">
                <div className="flex items-center justify-between mb-4 ">
                  ...
                  {pin.owner && pin.owner._id === user._id && (
                    <button className="bg-red-500 text-white py-1 px-3 rounded cursor-pointer" onClick={deletePinHandler}>
                      <MdDelete />
                    </button>
                  )}
                </div>
                ...
                {edit && (
                  <button
                    style={{ width: "200px" }}
                    className="bg-red-500 text-white px-3 py-1 mt-2 mb-2"
                ...
                <hr className="font-bold text-gray-400 mt-3 mb-3"></hr>
                  ...
                  {pin.comments && pin.comments.length > 0 ? (
                    pin.comments.map((e, i) => (
                      <div className="flex items-center justify-between mb-4" key={i}>
                        ...
                        {e.user===user._id&&<button className='bg-red-500 text-white py-1 px-3 rounded' onClick={()=>deleteCommentHandler(e._id)}><MdDelete/></button>}
                      </div>
                    ))
                  ) : (
